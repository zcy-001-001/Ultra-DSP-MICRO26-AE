 ...
 * Top module: W4A4_stage_s0_original
 * Precision: W4A4
 * Stage: s0_original
 * Source: Ultra-DSP-main/Area_Test_Ablation_Test/Ablation Experiment/Ablation Experiment.srcs/sources_1/new/W4A4_PD_Original.v
 * Toggle summary: runtime weight two's-complement conversion; combinational C-port; per-result two's-complement sign recovery
 */

module W4A4_stage_s0_original(
...
wire [3:0] w1_temp_mag, w2_temp_mag, w3_temp_mag, w4_temp_mag, w5_temp_mag, w6_temp_mag, w7_temp_mag;

assign w1_sign = w1[3];
assign w1_temp_mag = w1[3] ? (~w1[3:0] + 1'b1) : w1[3:0];
assign w1_mag = (w1_temp_mag == 4'b1000) ? 3'b111 : w1_temp_mag[2:0];

assign w2_sign = w2[3];
assign w2_temp_mag = w2[3] ? (~w2[3:0] + 1'b1) : w2[3:0];
assign w2_mag = (w2_temp_mag == 4'b1000) ? 3'b111 : w2_temp_mag[2:0];

assign w3_sign = w3[3];
assign w3_temp_mag = w3[3] ? (~w3[3:0] + 1'b1) : w3[3:0];
assign w3_mag = (w3_temp_mag == 4'b1000) ? 3'b111 : w3_temp_mag[2:0];

assign w4_sign = w4[3];
assign w4_temp_mag = w4[3] ? (~w4[3:0] + 1'b1) : w4[3:0];
assign w4_mag = (w4_temp_mag == 4'b1000) ? 3'b111 : w4_temp_mag[2:0];

assign w5_sign = w5[3];
assign w5_temp_mag = w5[3] ? (~w5[3:0] + 1'b1) : w5[3:0];
assign w5_mag = (w5_temp_mag == 4'b1000) ? 3'b111 : w5_temp_mag[2:0];

assign w6_sign = w6[3];
assign w6_temp_mag = w6[3] ? (~w6[3:0] + 1'b1) : w6[3:0];
assign w6_mag = (w6_temp_mag == 4'b1000) ? 3'b111 : w6_temp_mag[2:0];

assign w7_sign = w7[3];
assign w7_temp_mag = w7[3] ? (~w7[3:0] + 1'b1) : w7[3:0];
assign w7_mag = (w7_temp_mag == 4'b1000) ? 3'b111 : w7_temp_mag[2:0];
...
wire [47:0] dsp_C = (mode == 0) ? {
    ...
    15'b0, LSB_Res9, 2'b0, LSB_Res8, 2'b0, LSB_Res7, 2'b0,
    LSB_Res6, 2'b0, LSB_Res5, 1'b0, LSB_Res4, 1'b0,
    LSB_Res3, 2'b0, LSB_Res2, 4'b0
} : {
    
    23'b0, LSB_Res7, 2'b0, LSB_Res6, 2'b0, LSB_Res5, 1'b0,
    LSB_Res4, 1'b0, LSB_Res3, 2'b0, LSB_Res2, 4'b0
};
