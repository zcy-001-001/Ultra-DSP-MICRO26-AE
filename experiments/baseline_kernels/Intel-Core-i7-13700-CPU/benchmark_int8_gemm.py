...
=======================================================
Target: Intel Core i7-13700 (13th Gen Raptor Lake, AVX-512 VNNI)
Context: CPU baseline for Ultra-DSP FPGA DSP packing evaluation

Backend: Intel MKL cblas_gemm_s8u8s32 via Fortran interface
  - Directly calls MKL's VNNI-optimized INT8 GEMM kernel
  - Verified ~1.8 TOPS on i7-13700 (vs 0.09 TOPS with ONNX Runtime MatMulInteger)
...
def _candidate_mkl_paths():
    """Yield MKL runtime candidates without embedding machine-local paths."""
    ...
    for prefix in (os.environ.get("CONDA_PREFIX"), sys.prefix):
        if not prefix:
            continue
        if os.name == "nt":
            yield os.path.join(prefix, "Library", "bin", "mkl_rt.2.dll")
            yield os.path.join(prefix, "Library", "bin", "mkl_rt.dll")
        else:
            yield os.path.join(prefix, "lib", "libmkl_rt.so")
...
def _load_mkl():
    """Load MKL and configure the gemm_s8u8s32 Fortran interface."""
    errors = []
    for candidate in _candidate_mkl_paths():
        try:
            mkl = ctypes.cdll.LoadLibrary(candidate)
            break
        except OSError as exc:
            errors.append(f"{candidate}: {exc}")
    else:
        raise RuntimeError(
            "Unable to load MKL runtime. Install Intel MKL or set MKL_RT_PATH "
            "to the mkl_rt shared library. Tried:\n" + "\n".join(errors)
        )
    gemm = mkl.gemm_s8u8s32
    gemm.restype = None
    gemm.argtypes = [
...
class MKLInt8GEMM:
    ...
    Computes C_int32 = A_int8 @ B_int8 using:
    ...
    def __init__(self, M, K, N):
        ...
        self._lda = ctypes.c_int(M)   # column-major: lda = nrows
...
def main():
    ...
    parser.add_argument("--output-csv", type=str, default="results/gemm_benchmark.csv",
                        help="Output CSV path")
    parser.add_argument("--output-json", type=str, default="results/gemm_benchmark.json",
    ...
    print("  Model: LLaMA-2-7B | Data: INT8 (symmetric, zero_point=0)")
    print("  Backend: Intel MKL gemm_s8u8s32 (VNNI-optimized)")
    ...
    if args.decode_steps:
        ...
        ds_json = "results/decode_sequence_benchmark.json"
        ds_csv = "results/decode_sequence_benchmark.csv"
