...
def find_gpu_csv() -> Path:
    ...
    for candidate in candidates:
        if candidate.exists():
            return candidate
    raise FileNotFoundError("cannot find int4_batch_streaming_gpu_paper_anchored.csv")
...
def build_plot_data() -> pd.DataFrame:
    ...
    return pd.concat([gpu_rows, fpga], ignore_index=True)
...
def main() -> None:
    ...
    out_csv = RESULTS / "gpu_fpga_batch_throughput_efficiency.csv"
    out_png = RESULTS / "gpu_fpga_batch_throughput_efficiency.png"
    out_pdf = RESULTS / "gpu_fpga_batch_throughput_efficiency.pdf"
    ...
    fig, axes = plt.subplots(1, 2, figsize=(8.3, 3.15), constrained_layout=False)
    fig.subplots_adjust(left=0.085, right=0.995, bottom=0.20, top=0.76, wspace=0.32)
    ...
    axes[0].set_ylim(1.2, 1e3)
    axes[0].yaxis.set_major_locator(LogLocator(base=10, numticks=4))

    handles, labels = axes[0].get_legend_handles_labels()
    fig.legend(handles, labels, loc="upper center", ncol=3, frameon=False, fontsize=12, bbox_to_anchor=(0.56, 0.98))
