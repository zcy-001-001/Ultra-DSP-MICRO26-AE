...
// W5A5_array black-box function reference model (NO mode)
...
// - result0..result3 keep one 10-bit result per PE using the same layout
//
...
void W5A5_array(
    ...
    ap_int<10> result0[NUM_PE],
    ap_int<10> result1[NUM_PE],
    ap_int<10> result2[NUM_PE],
    ap_int<10> result3[NUM_PE]) {
#pragma HLS inline off
    ...
    for (int col = 0; col < ARRAY_COLS; ++col) {
        for (int row = 0; row < ARRAY_ROWS; ++row) {
            ...
            result0[pe_index] = (ap_int<10>)w0[pe_index] * (ap_int<10>)a0_row;
            result1[pe_index] = (ap_int<10>)w1[pe_index] * (ap_int<10>)a0_row;
            result2[pe_index] = (ap_int<10>)w0[pe_index] * (ap_int<10>)a1_row;
            result3[pe_index] = (ap_int<10>)w1[pe_index] * (ap_int<10>)a1_row;
        }
    }
}
