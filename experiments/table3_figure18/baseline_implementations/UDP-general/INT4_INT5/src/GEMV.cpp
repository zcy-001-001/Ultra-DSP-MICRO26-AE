...
void W4A5_array(
    ap_int<4> w0[NUM_DSP],
    ap_int<4> w1[NUM_DSP],
    ap_int<5> a0[DSP_ARRAY_ROWS],
    ap_int<5> a1[DSP_ARRAY_ROWS],
    ap_int<9> result0[NUM_DSP],
    ap_int<9> result1[NUM_DSP],
    ap_int<9> result2[NUM_DSP],
    ap_int<9> result3[NUM_DSP]);
...
PACK_LOCAL_COL:
    for (int local_col = 0; local_col < LOGICAL_COLS_PER_WEIGHT_PORT; ++local_col) {
        ...
        if (logical_col < LOGICAL_OUTPUT_COLS) {
            const int physical_col = logical_col / WEIGHTS_PER_PE;
            const int lane = logical_col % WEIGHTS_PER_PE;
...
#pragma HLS PIPELINE II=1
            ...
            if (lane == 0) {
                ...
                sum_a1 += (ap_int<32>)result2[pe_index];
            } else {
...
extern "C" void gemv_kernel(
    ...
    static ap_int<5> a0_arr[DSP_ARRAY_ROWS];
    static ap_int<5> a1_arr[DSP_ARRAY_ROWS];
    static ap_int<9> result0_arr[NUM_DSP];
    static ap_int<9> result1_arr[NUM_DSP];
    static ap_int<9> result2_arr[NUM_DSP];
    static ap_int<9> result3_arr[NUM_DSP];
