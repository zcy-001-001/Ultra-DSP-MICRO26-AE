...
static ap_int<8> encode_result(ap_int<4> weight, ap_int<5> act) {
    const ap_uint<1> sign = weight[3] ^ act[4];
    const ap_uint<3> w_mag = ((ap_uint<4>)weight).range(2, 0);
    const ap_uint<4> a_mag = abs_act(act);
    const ap_uint<7> magnitude = (ap_uint<7>)w_mag * (ap_uint<7>)a_mag;
    ap_uint<8> payload = 0;
    payload.range(6, 0) = magnitude;
    const ap_uint<8> encoded = sign ? (payload ^ ~ap_uint<8>(0)) : payload;
    return (ap_int<8>)encoded;
}

static ap_uint<1> encode_sign(ap_int<4> weight, ap_int<5> act) {
    return weight[3] ^ act[4];
}
...
void W4A5_P(
    ...
    ap_int<8> result7[NUM_PE],
    ap_int<8> result8[NUM_PE],
...
#pragma HLS inline off
    ...
    for (int col = 0; col < ARRAY_COLS; ++col) {
        for (int row = 0; row < ARRAY_ROWS; ++row) {
            ...
            result5[pe_index] = encode_result(w1[pe_index], a2_row);
            result6[pe_index] = encode_result(w2[pe_index], a2_row);
            result7[pe_index] = encode_result(w3[pe_index], a2_row);
            result8[pe_index] = encode_result(w4[pe_index], a2_row);
            ...
            sign5[pe_index] = encode_sign(w1[pe_index], a2_row);
            sign6[pe_index] = encode_sign(w2[pe_index], a2_row);
            sign7[pe_index] = encode_sign(w3[pe_index], a2_row);
            sign8[pe_index] = encode_sign(w4[pe_index], a2_row);
        }
    }
}
