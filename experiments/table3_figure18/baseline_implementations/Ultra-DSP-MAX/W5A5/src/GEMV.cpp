...
#pragma HLS PIPELINE II=1
            ...
            sign_a2 += (ap_int<32>)sign2[pe_index] +
                       (ap_int<32>)sign5[pe_index];
            sign_a3 += (ap_int<32>)sign3[pe_index] +
                       (ap_int<32>)sign6[pe_index];
        }

        output[col] = sum_a1 + sign_a1;
        output[DSP_ARRAY_COLS + col] = sum_a2 + sign_a2;
        output[(2 * DSP_ARRAY_COLS) + col] = sum_a3 + sign_a3;
    }
}

#define PACK_PORT(PORT_PTR, PORT_IDX) \
    pack_weight_port((PORT_PTR), (PORT_IDX), w1_arr, w2_arr)
...
extern "C" void gemv_kernel(
    ...
    static ap_int<8> result6_arr[NUM_DSP];
    ...
    load_activations(act_packed, a1_arr, a2_arr, a3_arr);
