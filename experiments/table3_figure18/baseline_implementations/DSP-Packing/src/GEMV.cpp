...
// One blackbox call fans out to 4096 corrected `FPL` leaves:
// - 64 rows x 64 physical columns = 4096 DSP-backed PEs
// - each PE computes 2 signed weight lanes x 3 activation lanes = 6 products
// - the leaf RTL already emits signed 9-bit two's-complement results, so the
//   kernel must not apply any extra sign/magnitude conversion
void FPL_4096(
    ap_int<5> w0[NUM_DSP],
    ap_int<5> w1[NUM_DSP],
    ap_uint<4> a0[DSP_ARRAY_ROWS],
    ap_uint<4> a1[DSP_ARRAY_ROWS],
    ap_uint<4> a2[DSP_ARRAY_ROWS],
    ap_int<9> result_a0w0[NUM_DSP],
    ap_int<9> result_a1w0[NUM_DSP],
    ap_int<9> result_a2w0[NUM_DSP],
    ap_int<9> result_a0w1[NUM_DSP],
    ap_int<9> result_a1w1[NUM_DSP],
    ap_int<9> result_a2w1[NUM_DSP]
);
...
#pragma HLS PIPELINE II=1
        ...
        a2[row] = act_packed[(2 * DSP_ARRAY_ROWS) + row];
    }
}

static void pack_weight_port(
    const ap_uint<5> *weight_port,
    int port_idx,
    ap_int<5> w0[NUM_DSP],
    ap_int<5> w1[NUM_DSP]) {
#pragma HLS INLINE
...
#pragma HLS PIPELINE II=1
            ...
            const int weight_idx = (local_col * DSP_ARRAY_ROWS) + row;
            const ap_int<5> weight_val = weight_port[weight_idx];
...
#define PACK_WEIGHT_PORT(PORT_PTR, PORT_IDX)     pack_weight_port((PORT_PTR), (PORT_IDX), w0_arr, w1_arr)
...
#pragma HLS INTERFACE m_axi port=weight_col31 offset=slave
    ...
    static ap_int<5> w0_arr[NUM_DSP];
    static ap_int<5> w1_arr[NUM_DSP];
    static ap_uint<4> a0_arr[DSP_ARRAY_ROWS];
    static ap_uint<4> a1_arr[DSP_ARRAY_ROWS];
    static ap_uint<4> a2_arr[DSP_ARRAY_ROWS];
    static ap_int<9> res_a0w0_arr[NUM_DSP];
    static ap_int<9> res_a1w0_arr[NUM_DSP];
    static ap_int<9> res_a2w0_arr[NUM_DSP];
    static ap_int<9> res_a0w1_arr[NUM_DSP];
    static ap_int<9> res_a1w1_arr[NUM_DSP];
    static ap_int<9> res_a2w1_arr[NUM_DSP];

    load_activations(act_packed, a0_arr, a1_arr, a2_arr);
    ...
    FPL_4096(
        ...
        res_a0w0_arr, res_a1w0_arr, res_a2w0_arr,
        res_a0w1_arr, res_a1w1_arr, res_a2w1_arr
    );
