...
def get_minq_maxq(bits, sym, narrow_symmetric=False):
    if sym:
        maxq = torch.tensor(2**(bits-1)-1)
        # Original full-range signed symmetric quantization uses -maxq - 1.
        # The rebuttal mixed-precision runs can request narrow symmetric
        # quantization, whose negative bound is exactly -maxq.
        minq = -maxq if narrow_symmetric else -maxq -1
    else:
...
def sym_quant(x, scale, minq, maxq):
    scale = scale.to(x.device)
...
def sym_quant_dequant(x, scale, minq, maxq):
    return sym_dequant(*sym_quant(x, scale, minq, maxq))
...
class WeightQuantizer(torch.nn.Module):
    ...
    def configure(
        ...
        mse=False, norm=2.4, grid=100, maxshrink=.8,pot=False,narrow_symmetric=False
    ):
        self.bits = bits
        ...
        if sym:
            ...
            self.minq = -self.maxq if narrow_symmetric else -self.maxq - 1
        else:
    ...
    def find_params(self, x):
        ...
        if self.sym:
            ...
            self.scale = torch.maximum((xmin.clip(max=0))/(self.minq) ,xmax.clip(min=1e-6)/(self.maxq))
        ...
        if self.mse:
            ...
            for i in range(int(self.maxshrink * self.grid)):
                ...
                if self.sym:
                    scale1 = torch.maximum((xmin1.clip(max=0))/(self.minq) ,xmax1.clip(min=1e-6)/(self.maxq))
                    
                    if self.pot:
                        scale1 = 2**(scale1.log2().ceil())
                    zero1 = torch.zeros_like(scale1)
                    q = sym_quant_dequant(x, scale1.unsqueeze(1), self.minq, self.maxq)
                else:
    ...
    def quantize(self, x):
        ...
        if self.ready() and self.bits < 16:
            if self.sym:
                return sym_quant_dequant(x, self.scale, self.minq, self.maxq).to(x_dtype)
        ...
        for names in sequential:
            ...
            for name in subset:
                ...
                gptq[name].quantizer.configure(
                    ...
                    narrow_symmetric=getattr(args, "narrow_symmetric", False)
                )
