...
// Convert two independent sign-magnitude products into one registered pair
// sum. The explicit LUT/CARRY8 network prevents synthesis from expanding the
// conditional negations into a wide mux/add structure.
...
// One independent 32-input tree consumes the registered first-level pair sums.
// These five registered fabric levels, together with the pair level above,
// form a balanced six-level 64-to-1 reduction at II=1.
(* keep_hierarchy = "yes", use_dsp = "no" *)
module w4a4_pure_lut_tree32 (
    input wire clk,
    input wire ce,
    input wire [32 * 8 - 1:0] pair_values,
    output wire [12:0] sum
);

wire [16 * 9 - 1:0] level16;
wire [8 * 10 - 1:0] level8;
wire [4 * 11 - 1:0] level4;
wire [2 * 12 - 1:0] level2;

genvar level16_node;
generate
    for (level16_node = 0; level16_node < 16;
         level16_node = level16_node + 1) begin : level16_gen
        w4a4_registered_fabric_add #(.INPUT_WIDTH(8)) add (
            .clk(clk), .ce(ce),
            .lhs(pair_values[(2 * level16_node + 0) * 8 +: 8]),
            .rhs(pair_values[(2 * level16_node + 1) * 8 +: 8]),
            .value(level16[level16_node * 9 +: 9])
        );
    end
endgenerate
...
genvar level8_node;
generate
    for (level8_node = 0; level8_node < 8;
         level8_node = level8_node + 1) begin : level8_gen
        w4a4_registered_fabric_add #(.INPUT_WIDTH(9)) add (
            .clk(clk), .ce(ce),
            .lhs(level16[(2 * level8_node + 0) * 9 +: 9]),
            .rhs(level16[(2 * level8_node + 1) * 9 +: 9]),
            .value(level8[level8_node * 10 +: 10])
        );
    end
endgenerate
...
genvar level4_node;
generate
    for (level4_node = 0; level4_node < 4;
         level4_node = level4_node + 1) begin : level4_gen
        w4a4_registered_fabric_add #(.INPUT_WIDTH(10)) add (
            .clk(clk), .ce(ce),
            .lhs(level8[(2 * level4_node + 0) * 10 +: 10]),
            .rhs(level8[(2 * level4_node + 1) * 10 +: 10]),
            .value(level4[level4_node * 11 +: 11])
        );
    end
endgenerate
...
genvar level2_node;
generate
    for (level2_node = 0; level2_node < 2;
         level2_node = level2_node + 1) begin : level2_gen
        w4a4_registered_fabric_add #(.INPUT_WIDTH(11)) add (
            .clk(clk), .ce(ce),
            .lhs(level4[(2 * level2_node + 0) * 11 +: 11]),
            .rhs(level4[(2 * level2_node + 1) * 11 +: 11]),
            .value(level2[level2_node * 12 +: 12])
        );
    end
endgenerate

w4a4_registered_fabric_add #(.INPUT_WIDTH(12)) level1_add (
    .clk(clk), .ce(ce),
    .lhs(level2[0 * 12 +: 12]),
    .rhs(level2[1 * 12 +: 12]),
    .value(sum)
...
// LUT/CARRY8 pair adders and registers all 32 pair sums. There are nine
// separate trees per column; no weight channel is merged here.
genvar level32_node;
generate
...
// Every channel/column owns one independent pure-LUT tree. Since TREE_INDEX is
// channel * 64 + column, each tree can be constrained with its multiplier
// column without any inter-column cascade.
genvar pure_tree_index;
generate
    for (pure_tree_index = 0; pure_tree_index < CHANNEL_SUMS;
         pure_tree_index = pure_tree_index + 1) begin : pure_tree_gen
        w4a4_pure_lut_tree32 tree (
            .clk(ap_clk), .ce(pipeline_ce),
            .pair_values(level32[
                pure_tree_index * PAIRS_PER_TREE * 8 +:
                PAIRS_PER_TREE * 8]),
            .sum(channel_sums[pure_tree_index * 13 +: 13])
        );
    end
endgenerate
