...
module tb_w4a4_overlap_depth5_hybrid;
reg clk = 1'b0;
reg mode = 1'b0;
reg [31:0] seed = 32'h0000130a;
reg [31:0] rand_word;
integer cycle = 0;
integer i;
integer failures = 0;
...
w4a4_overlap_depth5_hybrid dut(
    .clk(clk),
    .mode(mode),
    .w1(w1),
    .w2(w2),
    ...
    begin
        if (cycle >= 16) begin
            if (valid_count !== exp_valid_pipe[2]) begin
                $display("FAIL depth=5 cycle=%0d valid got=%0d expected=%0d", cycle, valid_count, exp_valid_pipe[2]);
                failures = failures + 1;
            end
            if (sign_bundle !== exp_sign_pipe[2]) begin
                $display("FAIL depth=5 cycle=%0d sign got=%b expected=%b", cycle, sign_bundle, exp_sign_pipe[2]);
                failures = failures + 1;
            end
            if (result_bundle !== exp_result_pipe[2]) begin
                $display("FAIL depth=5 cycle=%0d result got=%h expected=%h", cycle, result_bundle, exp_result_pipe[2]);
...
initial begin
    ...
    if (failures == 0) begin
        $display("PASS depth=5 top=w4a4_overlap_depth5_hybrid trials=64");
    end
    $finish;
end
