...
module tb_w4a4_overlap_depth3_hybrid;
reg clk = 1'b0;
reg mode = 1'b0;
reg [31:0] seed = 32'h00001308;
reg [31:0] rand_word;
integer cycle = 0;
integer i;
integer failures = 0;
...
w4a4_overlap_depth3_hybrid dut(
    .clk(clk),
    .mode(mode),
    .w1(w1),
    .w2(w2),
...
wire [62:0] result_bundle;
wire [8:0] sign_bundle;
...
reg [62:0] next_result;
reg [8:0] next_sign;
reg [4:0] next_valid;
reg [62:0] exp_result_pipe [0:2];
reg [8:0] exp_sign_pipe [0:2];
...
task compute_expected;
    begin
        ...
        if (mode == 1'b0) begin
            ...
            next_result[28 +: 7] = encode_product(w2, a2);
            next_sign[4] = w2[3] ^ a2[3];
            next_result[35 +: 7] = encode_product(w2, a3);
            next_sign[5] = w2[3] ^ a3[3];
            next_result[42 +: 7] = encode_product(w3, a1);
            next_sign[6] = w3[3] ^ a1[3];
            next_result[49 +: 7] = encode_product(w3, a2);
            next_sign[7] = w3[3] ^ a2[3];
            next_result[56 +: 7] = encode_product(w3, a3);
            next_sign[8] = w3[3] ^ a3[3];
        end
        else begin
            if (mode == 1'b1) begin
                next_valid = 5'd7;
    ...
    begin
        if (cycle >= 16) begin
            if (valid_count !== exp_valid_pipe[2]) begin
                $display("FAIL depth=3 cycle=%0d valid got=%0d expected=%0d", cycle, valid_count, exp_valid_pipe[2]);
                failures = failures + 1;
            end
            if (sign_bundle !== exp_sign_pipe[2]) begin
                $display("FAIL depth=3 cycle=%0d sign got=%b expected=%b", cycle, sign_bundle, exp_sign_pipe[2]);
                failures = failures + 1;
            end
            if (result_bundle !== exp_result_pipe[2]) begin
                $display("FAIL depth=3 cycle=%0d result got=%h expected=%h", cycle, result_bundle, exp_result_pipe[2]);
...
initial begin
    exp_result_pipe[0] = {63{1'b0}};
    exp_result_pipe[1] = {63{1'b0}};
    exp_result_pipe[2] = {63{1'b0}};
    exp_sign_pipe[0] = {9{1'b0}};
    exp_sign_pipe[1] = {9{1'b0}};
    exp_sign_pipe[2] = {9{1'b0}};
    ...
    if (failures == 0) begin
        $display("PASS depth=3 top=w4a4_overlap_depth3_hybrid trials=64");
    end
    $finish;
end
