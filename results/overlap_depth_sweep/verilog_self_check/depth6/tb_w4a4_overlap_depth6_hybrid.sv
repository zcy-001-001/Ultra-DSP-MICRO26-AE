...
module tb_w4a4_overlap_depth6_hybrid;
reg clk = 1'b0;
reg mode = 1'b0;
reg [31:0] seed = 32'h0000130b;
reg [31:0] rand_word;
integer cycle = 0;
integer i;
integer failures = 0;
...
w4a4_overlap_depth6_hybrid dut(
    .clk(clk),
    .mode(mode),
    .w1(w1),
    .w2(w2),
...
wire [139:0] result_bundle;
wire [19:0] sign_bundle;
...
assign result_bundle[84 +: 7] = result13;
assign sign_bundle[12] = sign13;
assign result_bundle[91 +: 7] = result14;
assign sign_bundle[13] = sign14;
assign result_bundle[98 +: 7] = result15;
assign sign_bundle[14] = sign15;
assign result_bundle[105 +: 7] = result16;
assign sign_bundle[15] = sign16;
assign result_bundle[112 +: 7] = result17;
assign sign_bundle[16] = sign17;
assign result_bundle[119 +: 7] = result18;
assign sign_bundle[17] = sign18;
assign result_bundle[126 +: 7] = result19;
assign sign_bundle[18] = sign19;
assign result_bundle[133 +: 7] = result20;
assign sign_bundle[19] = sign20;

reg [139:0] next_result;
reg [19:0] next_sign;
reg [4:0] next_valid;
reg [139:0] exp_result_pipe [0:2];
reg [19:0] exp_sign_pipe [0:2];
...
task compute_expected;
    begin
        ...
        if (mode == 1'b0) begin
            ...
            next_result[42 +: 7] = encode_product(w1, a5);
            next_sign[6] = w1[3] ^ a5[3];
            next_result[49 +: 7] = encode_product(w2, a3);
            next_sign[7] = w2[3] ^ a3[3];
            next_result[56 +: 7] = encode_product(w3, a1);
            next_sign[8] = w3[3] ^ a1[3];
            next_result[63 +: 7] = encode_product(w2, a4);
            next_sign[9] = w2[3] ^ a4[3];
            next_result[70 +: 7] = encode_product(w3, a2);
            next_sign[10] = w3[3] ^ a2[3];
            next_result[77 +: 7] = encode_product(w2, a5);
            next_sign[11] = w2[3] ^ a5[3];
            next_result[84 +: 7] = encode_product(w3, a3);
            next_sign[12] = w3[3] ^ a3[3];
            next_result[91 +: 7] = encode_product(w4, a1);
            next_sign[13] = w4[3] ^ a1[3];
            next_result[98 +: 7] = encode_product(w3, a4);
            next_sign[14] = w3[3] ^ a4[3];
            next_result[105 +: 7] = encode_product(w4, a2);
            next_sign[15] = w4[3] ^ a2[3];
            next_result[112 +: 7] = encode_product(w3, a5);
            next_sign[16] = w3[3] ^ a5[3];
            next_result[119 +: 7] = encode_product(w4, a3);
            next_sign[17] = w4[3] ^ a3[3];
            next_result[126 +: 7] = encode_product(w4, a4);
            next_sign[18] = w4[3] ^ a4[3];
            next_result[133 +: 7] = encode_product(w4, a5);
            next_sign[19] = w4[3] ^ a5[3];
        end
        else begin
            if (mode == 1'b1) begin
                next_valid = 5'd7;
    ...
    begin
        if (cycle >= 16) begin
            if (valid_count !== exp_valid_pipe[2]) begin
                $display("FAIL depth=6 cycle=%0d valid got=%0d expected=%0d", cycle, valid_count, exp_valid_pipe[2]);
                failures = failures + 1;
            end
            if (sign_bundle !== exp_sign_pipe[2]) begin
                $display("FAIL depth=6 cycle=%0d sign got=%b expected=%b", cycle, sign_bundle, exp_sign_pipe[2]);
                failures = failures + 1;
            end
            if (result_bundle !== exp_result_pipe[2]) begin
                $display("FAIL depth=6 cycle=%0d result got=%h expected=%h", cycle, result_bundle, exp_result_pipe[2]);
...
initial begin
    exp_result_pipe[0] = {140{1'b0}};
    exp_result_pipe[1] = {140{1'b0}};
    exp_result_pipe[2] = {140{1'b0}};
    exp_sign_pipe[0] = {20{1'b0}};
    exp_sign_pipe[1] = {20{1'b0}};
    exp_sign_pipe[2] = {20{1'b0}};
    ...
    if (failures == 0) begin
        $display("PASS depth=6 top=w4a4_overlap_depth6_hybrid trials=64");
    end
    $finish;
end
